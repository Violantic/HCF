...
public enum TagReplacer {
    ...
    protected String getValue() {
        switch (this) {
            ...
            case MAX_ALLIES:
                if (P.f.getInstance().getConfig().getBoolean("max-relations.enabled", true)) {
                    return String.valueOf(P.f.getInstance().getConfig().getInt("max-relations.ally", 10));
                }
                return TL.GENERIC_INFINITY.toString();
            case MAX_ENEMIES:
                if (P.f.getInstance().getConfig().getBoolean("max-relations.enabled", true)) {
                    return String.valueOf(P.f.getInstance().getConfig().getInt("max-relations.enemy", 10));
                }
                return TL.GENERIC_INFINITY.toString();
            case MAX_WARPS:
                return String.valueOf(P.f.getInstance().getConfig().getInt("max-warps", 5));
        }
        return null;
    }
    ...
    protected String getValue(Faction fac, FPlayer fp) {
        ...
        boolean minimal = P.f.getInstance().getConfig().getBoolean("minimal-show", false);

        if (fp != null) {
            switch (this) {
                case HEADER:
                    return P.f.txt.titleize(fac.getTag(fp));
                case PLAYER_NAME:
                    return fp.getName();
                case FACTION:
                ...
                case PLAYER_GROUP:
                    return P.f.getPrimaryGroup(Bukkit.getOfflinePlayer(UUID.fromString(fp.getId())));
                case PLAYER_BALANCE:
        ...
        switch (this) {
            ...
            case RAIDABLE:
                boolean raid = P.f.getInstance().getConfig().getBoolean("hcf.raidable", false) && fac.getLandRounded() >= fac.getPowerRounded();
    ...
    protected static List<com.massivecraft.factions.zcore.util.TagReplacer> getByType(TagType type) {
        List<com.massivecraft.factions.zcore.util.TagReplacer> tagReplacers = new ArrayList<com.massivecraft.factions.zcore.util.TagReplacer>();
