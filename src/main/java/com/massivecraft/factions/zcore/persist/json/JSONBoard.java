...
public class JSONBoard extends MemoryBoard {
    private static transient File file = new File(P.f.getInstance().getDataFolder(), "board.json");
    ...
    public void forceSave(boolean sync) {
        DiscUtil.writeCatch(file, P.f.gson.toJson(dumpAsSaveFormat()), sync);
    }

    public boolean load() {
        P.f.log("Loading board from disk");

        if (!file.exists()) {
            P.f.log("No board to load from disk. Creating new file.");
            forceSave();
            return true;
        }
        ...
        try {
            ...
            Map<String, Map<String, String>> worldCoordIds = P.f.gson.fromJson(DiscUtil.read(file), type);
            loadFromSaveFormat(worldCoordIds);
            P.f.log("Loaded " + flocationIds.size() + " board locations");
        } catch (Exception e) {
            e.printStackTrace();
            P.f.log("Failed to load the board from disk.");
            return false;
        }
