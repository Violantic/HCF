...
public class JSONFPlayers extends MemoryFPlayers {
    ...
    public JSONFPlayers() {
        file = new File(P.f.getInstance().getDataFolder(), "players.json");
        gson = P.f.gson;
    }

    public void convertFrom(MemoryFPlayers old) {
        this.fPlayers.putAll(Maps.transformValues(old.fPlayers, new Function<FPlayer, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>() {
            @Override
            public com.massivecraft.factions.zcore.persist.json.JSONFPlayer apply(FPlayer arg0) {
                return new com.massivecraft.factions.zcore.persist.json.JSONFPlayer((MemoryFPlayer) arg0);
            }
        }));
        forceSave();
    ...
    public void forceSave(boolean sync) {
        final Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer> entitiesThatShouldBeSaved = new HashMap<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>();
        for (FPlayer entity : this.fPlayers.values()) {
            if (((MemoryFPlayer) entity).shouldBeSaved()) {
                entitiesThatShouldBeSaved.put(entity.getId(), (com.massivecraft.factions.zcore.persist.json.JSONFPlayer) entity);
            }
        }
    ...
    private boolean saveCore(File target, Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer> data, boolean sync) {
        return DiscUtil.writeCatch(target, this.gson.toJson(data), sync);
    }

    public void load() {
        Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer> fplayers = this.loadCore();
        if (fplayers == null) {
            return;
        }
        this.fPlayers.clear();
        this.fPlayers.putAll(fplayers);
        P.f.log("Loaded " + fPlayers.size() + " players");
    }

    private Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer> loadCore() {
        if (!this.file.exists()) {
            return new HashMap<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>();
        }
        ...
        Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer> data = this.gson.fromJson(content, new TypeToken<Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>>() {
        }.getType());
        Set<String> list = new HashSet<String>();
        Set<String> invalidList = new HashSet<String>();
        for (Entry<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer> entry : data.entrySet()) {
        ...
        if (list.size() > 0) {
            ...
            saveCore(file, (Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>) data, true);
            ...
            try {
                ...
                for (String value : response.keySet()) {
                    ...
                    com.massivecraft.factions.zcore.persist.json.JSONFPlayer player = data.get(value);
            ...
            saveCore(this.file, (Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>) data, true); // Update the
            // flatfile
            Bukkit.getLogger().log(Level.INFO, "Done converting players.json to UUID.");
        }
        return (Map<String, com.massivecraft.factions.zcore.persist.json.JSONFPlayer>) data;
    }
