...
public abstract class MemoryFPlayer implements FPlayer {
    ...
    public void setAutoLeave(boolean willLeave) {
        ...
        P.f.debug(name + " set autoLeave to " + willLeave);
    }
    ...
    public MemoryFPlayer(String id) {
        ...
        this.powerBoost = 0.0;
        this.showScoreboard = P.f.getInstance().getConfig().getBoolean("scoreboard.default-enabled", false);
        this.kills = 0;
        this.deaths = 0;
    ...
    public MemoryFPlayer(com.massivecraft.factions.zcore.persist.MemoryFPlayer other) {
        ...
        this.showScoreboard = P.f.getInstance().getConfig().getBoolean("scoreboard.default-enabled", true);
    ...
    public void sendFactionHereMessage(Faction from) {
        ...
        if (showInfoBoard(toShow)) {
            ...
            showChat = P.f.getInstance().getConfig().getBoolean("scoreboard.also-send-chat", true);
        }
        if (showChat) {
            this.sendMessage(P.f.txt.parse(TL.FACTION_LEAVE.format(from.getTag(this), toShow.getTag(this))));
        }
    }
    ...
    public boolean showInfoBoard(Faction toShow) {
        return showScoreboard && !toShow.isWarZone() && !toShow.isWilderness() && !toShow.isSafeZone() && P.f.getInstance().getConfig().contains("scoreboard.finfo") && P.f.getInstance().getConfig().getBoolean("scoreboard.finfo-enabled", false) && FScoreboard.get(this) != null;
    }
    ...
    public void leave(boolean makePay) {
        ...
        if (myFaction.isNormal()) {
            ...
            if (Conf.logFactionLeave) {
                P.f.log(TL.LEAVE_LEFT.format(this.getName(), myFaction.getTag()));
            }
        }
        ...
        if (myFaction.isNormal() && !perm && myFaction.getFPlayers().isEmpty()) {
            ...
            if (Conf.logFactionDisband) {
                P.f.log(TL.LEAVE_DISBANDEDLOG.format(myFaction.getTag(), myFaction.getId(), this.getName()));
            }
        }
    }
    ...
    public boolean canClaimForFactionAtLocation(Faction forFaction, Location location, boolean notifyFailure) {
        ...
        int factionBuffer = P.f.getInstance().getConfig().getInt("hcf.buffer-zone", 0);
        int worldBuffer = P.f.getInstance().getConfig().getInt("world-border.buffer", 0);

        if (Conf.worldGuardChecking && Worldguard.checkForRegionsInChunk(location)) {
            // Checks for WorldGuard regions in the chunk attempting to be claimed
            error = P.f.txt.parse(TL.CLAIM_PROTECTED.toString());
        } else if (Conf.worldsNoClaiming.contains(flocation.getWorldName())) {
            error = P.f.txt.parse(TL.CLAIM_DISABLED.toString());
            ...
            error = P.f.txt.parse(TL.CLAIM_CANTCLAIM.toString(), forFaction.describeTo(this));
        } else if (forFaction == currentFaction) {
            error = P.f.txt.parse(TL.CLAIM_ALREADYOWN.toString(), forFaction.describeTo(this, true));
        } else if (this.getRole().value < Role.MODERATOR.value) {
            error = P.f.txt.parse(TL.CLAIM_MUSTBE.toString(), Role.MODERATOR.getTranslation());
        } else if (forFaction.getFPlayers().size() < Conf.claimsRequireMinFactionMembers) {
            error = P.f.txt.parse(TL.CLAIM_MEMBERS.toString(), Conf.claimsRequireMinFactionMembers);
        } else if (currentFaction.isSafeZone()) {
            error = P.f.txt.parse(TL.CLAIM_SAFEZONE.toString());
        } else if (currentFaction.isWarZone()) {
            error = P.f.txt.parse(TL.CLAIM_WARZONE.toString());
        } else if (P.f.getInstance().getConfig().getBoolean("hcf.allow-overclaim", true) && ownedLand >= forFaction.getPowerRounded()) {
            error = P.f.txt.parse(TL.CLAIM_POWER.toString());
        } else if (Conf.claimedLandsMax != 0 && ownedLand >= Conf.claimedLandsMax && forFaction.isNormal()) {
            error = P.f.txt.parse(TL.CLAIM_LIMIT.toString());
        } else if (currentFaction.getRelationTo(forFaction) == Relation.ALLY) {
            error = P.f.txt.parse(TL.CLAIM_ALLY.toString());
            ...
            if (Conf.claimsCanBeUnconnectedIfOwnedByOtherFaction) {
                error = P.f.txt.parse(TL.CLAIM_CONTIGIOUS.toString());
            } else {
                error = P.f.txt.parse(TL.CLAIM_FACTIONCONTIGUOUS.toString());
            }
        } else if (factionBuffer > 0 && Board.getInstance().hasFactionWithin(flocation, myFaction, factionBuffer)) {
            error = P.f.txt.parse(TL.CLAIM_TOOCLOSETOOTHERFACTION.format(factionBuffer));
        } else if (flocation.isOutsideWorldBorder(worldBuffer)) {
            if (worldBuffer > 0) {
                error = P.f.txt.parse(TL.CLAIM_OUTSIDEBORDERBUFFER.format(worldBuffer));
            } else {
                error = P.f.txt.parse(TL.CLAIM_OUTSIDEWORLDBORDER.toString());
            }
        } else if (currentFaction.isNormal()) {
            if (myFaction.isPeaceful()) {
                error = P.f.txt.parse(TL.CLAIM_PEACEFUL.toString(), currentFaction.getTag(this));
            } else if (currentFaction.isPeaceful()) {
                error = P.f.txt.parse(TL.CLAIM_PEACEFULTARGET.toString(), currentFaction.getTag(this));
            } else if (!currentFaction.hasLandInflation()) {
                // TODO more messages WARN current faction most importantly
                error = P.f.txt.parse(TL.CLAIM_THISISSPARTA.toString(), currentFaction.getTag(this));
            } else if (currentFaction.hasLandInflation() && !P.f.getInstance().getConfig().getBoolean("hcf.allow-overclaim", true)) {
                // deny over claim when it normally would be allowed.
                error = P.f.txt.parse(TL.CLAIM_OVERCLAIM_DISABLED.toString());
            } else if (!Board.getInstance().isBorderLocation(flocation)) {
                error = P.f.txt.parse(TL.CLAIM_BORDER.toString());
            }
        }
    ...
    public boolean attemptClaim(Faction forFaction, Location location, boolean notifyFailure) {
        ...
        if (Conf.logLandClaims) {
            P.f.log(TL.CLAIM_CLAIMEDLOG.toString(), this.getName(), flocation.getCoordString(), forFaction.getTag());
        }
    ...
    public void msg(String str, Object... args) {
        this.sendMessage(P.f.txt.parse(str, args));
    }
