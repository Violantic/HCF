...
public class JSONFactions extends MemoryFactions {
    ...
    public JSONFactions() {
        this.file = new File(P.f.getInstance().getDataFolder(), "factions.json");
        this.gson = P.f.gson;
        this.nextId = 1;
    }
    ...
    public void forceSave(boolean sync) {
        final Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction> entitiesThatShouldBeSaved = new HashMap<String, com.massivecraft.factions.zcore.persist.json.JSONFaction>();
        for (Faction entity : this.factions.values()) {
            entitiesThatShouldBeSaved.put(entity.getId(), (com.massivecraft.factions.zcore.persist.json.JSONFaction) entity);
        }

        saveCore(file, entitiesThatShouldBeSaved, sync);
    }

    private boolean saveCore(File target, Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction> entities, boolean sync) {
        return DiscUtil.writeCatch(target, this.gson.toJson(entities), sync);
    }

    public void load() {
        Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction> factions = this.loadCore();
        if (factions == null) {
            return;
        }
        ...
        super.load();
        P.f.log("Loaded " + factions.size() + " Factions");
    }

    private Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction> loadCore() {
        if (!this.file.exists()) {
            return new HashMap<String, com.massivecraft.factions.zcore.persist.json.JSONFaction>();
        }
        ...
        Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction> data = this.gson.fromJson(content, new TypeToken<Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction>>() {
        }.getType());
        ...
        int needsUpdate = 0;
        for (Entry<String, com.massivecraft.factions.zcore.persist.json.JSONFaction> entry : data.entrySet()) {
        ...
        if (needsUpdate > 0) {
            ...
            saveCore(file, (Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction>) data, true);
            ...
            saveCore(this.file, (Map<String, com.massivecraft.factions.zcore.persist.json.JSONFaction>) data, true); // Update the flatfile
    ...
    public Faction generateFactionObject() {
        String id = getNextId();
        Faction faction = new com.massivecraft.factions.zcore.persist.json.JSONFaction(id);
    ...
    public void convertFrom(MemoryFactions old) {
        this.factions.putAll(Maps.transformValues(old.factions, new Function<Faction, com.massivecraft.factions.zcore.persist.json.JSONFaction>() {
            @Override
            public com.massivecraft.factions.zcore.persist.json.JSONFaction apply(Faction arg0) {
