...
//		if ( ! lib.require("gson.jar", "http://search.maven.org/remotecontent?filepath=com/google/code/gson/gson/2.1/gson-2.1.jar")) return false;
        ...
        try {
            Map<String, Map<String, Object>> refCmd = this.getInstance().getDescription().getCommands();
            if (refCmd != null && !refCmd.isEmpty()) {
                this.refCommand = (String) (refCmd.keySet().toArray()[0]);
                System.out.println(refCommand);
                System.out.println("FACTIONS registering " + refCmd.size() + " commands");
            }
        ...
        getInstance().getServer().getPluginManager().registerEvents(this.mPluginSecretPlayerListener, getInstance());
        ...
        if (saveTask == null && Conf.saveToFileEveryXMinutes > 0.0) {
            ...
            saveTask = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(getInstance(), new SaveTask(this), saveTicks, saveTicks);
        }
    ...
    public void loadLang() {
        File lang = new File(getInstance().getDataFolder(), "lang.yml");
        OutputStream out = null;
        InputStream defLangStream = this.getInstance().getResource("lang.yml");
        if (!lang.exists()) {
            try {
                ...
                getInstance().getLogger().severe("[Factions] Couldn't create language file.");
                getInstance().getLogger().severe("[Factions] This is a fatal error. Now disabling");
                //this.getInstance().setEnabled(false); // Without it loaded, we can't send them messages
            } finally {
                if (defLangStream != null) {
                    try {
                        defLangStream.close();
        ...
        try {
            ...
            getInstance().getLogger().log(Level.WARNING, "Factions: Failed to save lang.yml.");
            getInstance().getLogger().log(Level.WARNING, "Factions: Report this stack trace to drtshock.");
            e.printStackTrace();
        }
    }
    ...
    public void onDisable() {
        if (saveTask != null) {
            this.getInstance().getServer().getScheduler().cancelTask(saveTask);
            saveTask = null;
        }
    ...
    public void suicide() {
        log("Now I suicide!");
        this.getInstance().getServer().getPluginManager().disablePlugin(getInstance());
    }
    ...
    public void log(Level level, Object msg) {
        Bukkit.getLogger().log(level, "[" + this.getInstance().getDescription().getFullName() + "] " + msg);
    }
}
