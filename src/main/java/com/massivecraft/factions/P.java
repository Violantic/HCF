...
public class P extends MPlugin {
    ...
    public void onEnable() {
        ...
        getInstance().getServer().getPluginManager().registerEvents(new FactionsPlayerListener(this), getInstance());
        getInstance().getServer().getPluginManager().registerEvents(new FactionsChatListener(this), getInstance());
        getInstance().getServer().getPluginManager().registerEvents(new FactionsEntityListener(this), getInstance());
        getInstance().getServer().getPluginManager().registerEvents(new FactionsExploitListener(), getInstance());
        getInstance().getServer().getPluginManager().registerEvents(new FactionsBlockListener(this), getInstance());

        // since some other plugins execute commands directly through this command interface, provide it
        this.getInstance().getCommand(this.refCommand).setExecutor(getInstance());
    ...
    private boolean setupPlayervaults() {
        Plugin plugin = getInstance().getServer().getPluginManager().getPlugin("PlayerVaults");
    ...
    public void onDisable() {
        ...
        if (AutoLeaveTask != null) {
            this.getInstance().getServer().getScheduler().cancelTask(AutoLeaveTask);
            AutoLeaveTask = null;
        }
    ...
    public void startAutoLeaveTask(boolean restartIfRunning) {
        ...
        if (Conf.autoLeaveRoutineRunsEveryXMinutes > 0.0) {
            ...
            AutoLeaveTask = getInstance().getServer().getScheduler().scheduleSyncRepeatingTask(getInstance(), new AutoLeaveTask(), ticks, ticks);
        }
    }
    ...
    public void debug(Level level, String s) {
        if (getInstance().getConfig().getBoolean("debug", false)) {
            getInstance().getLogger().log(level, s);
        }
    }
