...
public class Econ {
    ...
    public static void setup() {
        ...
        if (Bukkit.getServer().getPluginManager().getPlugin("Vault") == null) {
            P.f.log(integrationFail + "is not installed.");
            return;
        }

        RegisteredServiceProvider<Economy> rsp = Bukkit.getServer().getServicesManager().getRegistration(Economy.class);
        if (rsp == null) {
            P.f.log(integrationFail + "is not hooked into an economy plugin.");
            return;
        }
        econ = rsp.getProvider();

        P.f.log("Economy integration through Vault plugin successful.");

        if (!Conf.econEnabled) {
            P.f.log("NOTE: Economy is disabled. You can enable it with the command: f config econEnabled true");
        }

        P.f.cmdBase.cmdHelp.updateHelp();
    }
    ...
    public static void sendBalanceInfo(FPlayer to, EconomyParticipator about) {
        if (!shouldBeUsed()) {
            P.f.log(Level.WARNING, "Vault does not appear to be hooked into an economy plugin.");
            return;
        }
