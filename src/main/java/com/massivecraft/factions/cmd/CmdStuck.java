...
public class CmdStuck extends FCommand {
    ...
    public void perform() {
        ...
        if (P.f.getStuckMap().containsKey(player.getUniqueId())) {
            long wait = P.f.getTimers().get(player.getUniqueId()) - System.currentTimeMillis();
            ...
            final int id = Bukkit.getScheduler().runTaskLater(P.p, new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (world.getUID() != player.getWorld().getUID() || sentAt.distance(player.getLocation()) > radius) {
                        ...
                        P.f.getTimers().remove(player.getUniqueId());
                        P.f.getStuckMap().remove(player.getUniqueId());
                        return;
                    }
                        ...
                        public boolean work() {
                            ...
                            if (faction.isWilderness()) {
                                ...
                                if (!Essentials.handleTeleport(player, tp)) {
                                    player.teleport(tp);
                                    P.f.debug("/f stuck used regular teleport, not essentials!");
                                }
                                this.stop();
                                return false;
                            }
            ...
            P.f.getTimers().put(player.getUniqueId(), System.currentTimeMillis() + (delay * 1000));
            long wait = P.f.getTimers().get(player.getUniqueId()) - System.currentTimeMillis();
            String time = DurationFormatUtils.formatDuration(wait, TL.COMMAND_STUCK_TIMEFORMAT.toString(), true);
            msg(TL.COMMAND_STUCK_START, time);
            P.f.getStuckMap().put(player.getUniqueId(), id);
        }
    }
