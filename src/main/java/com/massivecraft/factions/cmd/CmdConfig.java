    ...
    public void perform() {
        ...
        if (!success.isEmpty()) {
            if (sender instanceof Player) {
                sendMessage(success);
                P.f.log(success + TL.COMMAND_CONFIG_LOG.format((Player) sender));
            } else  // using P.f.log() instead of sendMessage if run from server console so that "[Factions v#.#.#]" is prepended in server log
            {
                P.f.log(success);
            }
        }
