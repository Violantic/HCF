...
public class FTeamWrapper {
    private static final Map<Faction, com.massivecraft.factions.scoreboards.FTeamWrapper> wrappers = new HashMap<Faction, com.massivecraft.factions.scoreboards.FTeamWrapper>();
    ...
    public static void applyUpdatesLater(final Faction faction) {
        ...
        if (!P.p.getInstance().getConfig().getBoolean("scoreboard.default-prefixes", false)) {
            return;
        }


        if (updating.add(faction)) {
            Bukkit.getScheduler().runTask(P.p.getInstance(), new Runnable() {
                @Override
                public void run() {
