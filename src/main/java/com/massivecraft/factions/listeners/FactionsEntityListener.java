...
public class FactionsEntityListener implements Listener {
    ...
    public void onTravel(PlayerPortalEvent event) {
        if (!P.f.getInstance().getConfig().getBoolean("portals.limit", false)) {
        ...
        if (event.useTravelAgent() && agent.getCanCreatePortal() && agent.findPortal(event.getTo()) == null) {
            ...
            String mininumRelation = P.f.getInstance().getConfig().getString("portals.minimum-relation", "MEMBER"); // Defaults to Neutral if typed wrong.
